...
static uint8_t YUV_to_R[256][256];
static uint8_t YUV_to_G[256][256][256];
static uint8_t YUV_to_B[256][256];
...
{ 
  ...
  for(short i = 0; i<h; i++){
    for(short j = 0; j<w; j+=2){
      y_first_pixel = source[1];
      u_pixel = source[0];
      y_second_pixel = source[3];
      v_pixel = source[2];
      uint8_t R1 = YUV_to_R[y_first_pixel][v_pixel];
      uint8_t G1 = YUV_to_G[y_first_pixel][u_pixel][v_pixel];
      uint8_t B1 = YUV_to_B[y_first_pixel][u_pixel];
      uint8_t R2 = YUV_to_R[y_second_pixel][v_pixel];
      uint8_t G2 = YUV_to_G[y_second_pixel][u_pixel][v_pixel];
      uint8_t B2 = YUV_to_B[y_second_pixel][u_pixel];
    ...
    for (short v = 0; v < 256; v++) {
        for (short y = 0; y < 256; y++) {
            ...
            else if (temp_r < 0)
            {
              ...
              YUV_to_R[y][v] = (uint8_t)temp_r;
            }

            short temp_g = (y - ((352 * U + 731 * V) >> 10));
            if (temp_g > 255){
              YUV_to_G[y][u][v] = 255;
            }
            else if (temp_g < 0)
            {
              ...
              YUV_to_G[y][u][v] = (uint8_t)temp_g;
            }
            short temp_b = ( y + ((1814 * U) >> 10));
            if (temp_b > 255){
              YUV_to_B[y][u] = 255;
            }
            else if (temp_b < 0)
