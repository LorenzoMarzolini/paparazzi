...
// #include "svm_model_step_10x10.h"
#include "SIM_svm_model_step_10x10.h"
...
static int svm_predict_from_features(const uint8_t *input_features) {
  float score = 0.0f;
  for (int i = 0; i < SVM_NUM_FEATURES; ++i) {
    score += svm_weights[i] * input_features[i];
  }
  score += svm_bias;
...
{  
  ...
  uint8_t input_features[SVM_NUM_FEATURES];
  int feature_idx = 0;
  const int step_y = SVM_STEP_Y;
  const int step_x = SVM_STEP_X;

  for (int y = 0; y < h; y += step_y) {
    for (int x = 0; x < w - 1; x += step_x * 2) {
      int byte_index = y * w * 2 + (x & ~1) * 2;
      if (byte_index + 3 >= w * h * 2 || feature_idx + 3 >= SVM_NUM_FEATURES) break;

      uint8_t u = source[byte_index + 0];
      uint8_t y0 = source[byte_index + 1];
      uint8_t v = source[byte_index + 2];
      uint8_t y1 = source[byte_index + 3];

      input_features[feature_idx++] = u;
      input_features[feature_idx++] = y0;
      input_features[feature_idx++] = v;
      input_features[feature_idx++] = y1;
    }
  }

  int tree_detected = svm_predict_from_features(input_features);
  ...
  for(short i = 0; i<h; i++){
    for(short j = 0; j<w; j+=2){
      ...
      short u = (short)source[0];
      short y2 = (short)source[3];
      short v = (short)source[2];
      if(y1 < 196 && u < 114 && v<140){dest[0] = 255;} else {dest[0] = 0;};
      if(y2 < 196 && u < 114 && v<140){dest[1] = 255;} else {dest[1] = 0;};
      dest += 2;
      source += 4;
    }
  }
